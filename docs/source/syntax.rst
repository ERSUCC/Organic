...
in your program, you can then use the name :code:`frequency` to refer to the number :organic:mono:`440`.
...
in your program, you can then use the name :code:`frequency` to refer to the number :organic:mono:`124`, without needing
to type or evaluate the expression :code:`(2 * 12) + 100` again.
...
These variables will then have the values :organic:mono:`540` and :organic:mono:`640`, respectively.
...
The above example uses the function named :code:`sine`, assigning the value :code:`440` to the input named
:code:`frequency`. As you heard when you ran this example program earlier, this creates a sine wave with the specified
frequency; in this case, :organic:mono:`440` Hz. The parentheses after the name :code:`sine` tell Organic that you want
to use and configure a function, instead of using a variable.
...
is optional, and Organic will set it to :organic:mono:`1` (full volume) if you don't provide a value yourself. By
contrast, the :code:`frequency` input for the :code:`sine` function is required, and Organic won't let you run the
program if you forget to provide a value for it.
...
constant. For example, you can use the :code:`sweep` function to extend the original sine wave example:
...
done with the :code:`if` function, like in the following example:
...
and the :code:`repeat` function as you saw in the previous section to create a rising pitch that resets every
:organic:mono:`2` seconds. Then, it creates a variable named :code:`vol` to contain the volume, which is either
:organic:mono:`1` (full volume) or :organic:mono:`0` (silent) depending on the current frequency. Next, you can use
these variables as the inputs to the :code:`sine` function to hear how it sounds:
...
:organic:mono:`0` in the :code:`if` function, and see how it sounds compared to the first example. You should hear the
exact opposite: a moment of silence, followed by the upper portion of the rising tone.
...
time. To use it with an audio source, you can provide it as the value for that audio source's :code:`effects` input. To
hear what this sounds like, let's build up an example using what you've learned about controllers and effects. Start by
creating a basic saw wave at :organic:mono:`440` Hz:
